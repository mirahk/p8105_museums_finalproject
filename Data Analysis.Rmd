...
the plot of model residuals vs fitted values is clustered around 0, with the average predicted value being approximately 10. The cluster is pretty tight, however there are some outliars. 
...
The model excluding income has a much higher RMSE value than the model including income.
...
```{r}
museum_edu_df |>
  modelr::add_residuals(model_int) |>
  modelr::add_predictions(model_int) |>
  ggplot(aes(x=resid, y=pred)) + geom_point(alpha=0.5, size=0.5)
```

the plot of model residuals vs fitted values is clustered around 0, with the average predicted value being approximately 12. The cluster is pretty tight, however there are some outliars. It looks very similar to the plot of model residuals against fitted values for the model without the interaction term, however the cluster is slightly higher on the plot.
...
The model with an interaction term between bachelors and median income has a lower RMSE value than the model without an interaction term. Ultimately, the model with interaction terms appears to be the best model.